...
#define VERSION_ADDRESS                                                        \
  0x0800FC00 // Son sektörün bir bölümü — versiyon bilgisi saklanır

// ── AES Key Flash Storage (Sector 1 = 0x08004000, 16KB) ──
// Format: [MAGIC:4] [AES_KEY:32] [CRC32:4] = 40 byte
#define KEY_STORAGE_ADDR 0x08004000
#define KEY_STORAGE_SECTOR FLASH_SECTOR_1
#define KEY_MAGIC 0xDEADBEEF
#define KEY_UPDATE_MAGIC                                                       \
  0xA5A5A5A5 // Şifresi çözülen paketteki doğrulama değeri
...
// Watchdog timeout ~4 saniye (LSI ≈ 32kHz, prescaler=64 → 500Hz, reload=2000 →
// 4s)
...
// Varsayılan AES key (ilk boot veya flash bozuk olduğunda kullanılır)
...
// Aktif AES key — boot'ta flash'tan yüklenir
uint8_t AES_KEY[32];

uint8_t AES_IV[16] = {0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68,
 ...
 * Sector silme + yazma işlemi atomik değil; güç kesilirse varsayılan key'e
 * döner.
 */
static uint8_t save_key_to_flash(const uint8_t *new_key) {
  ...
  // Magic yaz
  if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, KEY_STORAGE_ADDR, KEY_MAGIC) !=
      HAL_OK) {
    HAL_FLASH_Lock();
    return 0;
  }
  ...
  // CRC yaz
  if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, KEY_STORAGE_ADDR + 36,
                        key_crc) != HAL_OK) {
    HAL_FLASH_Lock();
    return 0;
  }
...
// =========================================================================
// GÜVENLİ AES KEY GÜNCELLEMESİ
// =========================================================================
/**
 * Güvenli key güncelleme protokolü:
 *
 * 1. PC 'K' gönderir
 * 2. STM32 ACK gönderir
 * 3. PC şu paketi gönderir (68 byte):
 *    [IV:16] + [AES_CBC(current_key, IV, new_key(32) + 0xA5A5A5A5(4) +
 * padding(12)):48] + [CRC32:4]
 *
 * 4. STM32:
 *    a) CRC-32 doğrula (şifreli 48 byte üzerinden)
 *    b) Mevcut key ile şifre çöz
 *    c) Magic (0xA5A5A5A5) doğrula → doğru key ile şifrelendiğini kanıtlar
 *    d) Yeni key'i flash'a yaz
 *    e) RAM'deki aktif key'i güncelle
 *
 * Güvenlik:
...
void Bootloader_Handle_Secure_Write(void) {
  ...
  // ───
  HAL_StatusTypeDef meta_status =
      HAL_UART_Receive(&huart2, meta_buffer, 12, 10000);
  ...
  while (1) {
    ...
    if (status == HAL_OK) {
      ...
      for (int i = 0; i < 128; i += 4) {
        ...
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, current_addr, data) !=
            HAL_OK) {
          flash_error = 1;
          break;
        }
